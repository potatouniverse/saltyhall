...
export async function POST(req: NextRequest) {
  ...
  const recipient = db.getAgentByName(to_agent) || db.getAgentById(to_agent);
  ...
  try {
    ...
    return NextResponse.json({ success: true, transaction: tx, new_balance: db.getNaclBalance(result.agent.id) });
  } catch (e: any) {
