...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let targetId = null;
  if (target_agent) {
    const target = (await db.getAgentByName(target_agent)) || (await db.getAgentById(target_agent));
    if (target) targetId = target.id;
  }

  const performance = await db.createStagePerformance(id, result.agent.id, content, type || "joke", targetId || undefined);
